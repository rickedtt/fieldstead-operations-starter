...
function EmailView({ state }: { state: OperationsState }) {
  ...
  return <div className="settings-page email-page">
    ...
    {configured && <><section className="email-toolbar"><button className="primary email-compose-button" onClick={() => dispatchEmailUi({ type:'open-compose' })}>＋ Compose</button></section><section className="attention-card settings-card email-inbox-card"><div className="section-title"><div><p className="eyebrow">INBOX</p><h2>Incoming messages</h2></div><span className="email-count">{messages.length || ''}</span></div>{messages.length === 0 ? <Empty title="No synced messages yet" detail="Use Sync now to check the selected mailbox." /> : <div className="email-list">{messages.map((message) => { const expanded = emailUi.expandedMessageId === message.id; const attachmentCount = message.attachments?.length || 0; const imageCount = message.inlineImages?.length || 0; const detailId = `email-detail-${message.id}`; return <article key={message.id} className={cx('email-message-card', message.unread && 'email-unread', expanded && 'email-expanded')}><button type="button" className="email-message-summary" aria-expanded={expanded} aria-controls={detailId} onClick={() => dispatchEmailUi({ type:'toggle-message', messageId:message.id })}><strong className="email-sender">{message.starred && <span className="email-star" aria-label="Starred">★</span>}{message.fromName || message.from || 'Unknown sender'}</strong><span className="email-subject"><b>{message.subject || '(no subject)'}</b><em> — {message.text.replace(/\s+/g,' ').slice(0,150) || (attachmentCount ? `${attachmentCount} attachment${attachmentCount === 1 ? '' : 's'}` : 'No message preview')}</em></span><span className="email-row-indicators">{attachmentCount > 0 && <span title={`${attachmentCount} attachment${attachmentCount === 1 ? '' : 's'}`} aria-label={`${attachmentCount} attachment${attachmentCount === 1 ? '' : 's'}`}>📎</span>}{imageCount > 0 && <span title="Includes image" aria-label="Includes image">▧</span>}</span><small>{formatWhen(message.receivedAt)}</small><span className="email-chevron" aria-hidden="true">{expanded ? '⌃' : '⌄'}</span></button>{expanded && <div className="email-message-details" id={detailId}><div className="email-detail-heading"><strong>{message.subject || '(no subject)'}</strong><small>From {message.fromName || message.from || 'Unknown sender'}{message.fromName && message.from ? ` <${message.from}>` : ''}</small></div><EmailMessageContent message={message} onSaveAttachment={(attachment) => void saveAttachment(message, attachment)} />{matched(message) && <span className="pill pill-approved">Matched to {matched(message)?.name}</span>}<div className="email-message-actions"><button type="button" className="text-button" disabled={busy} onClick={() => prepareReply(message)}>Reply</button><button type="button" className="text-button" disabled={busy} onClick={() => prepareReply(message, true)}>Forward</button><button type="button" className="text-button" disabled={busy} onClick={() => void messageAction(message, message.unread ? 'read' : 'unread')}>{message.unread ? 'Mark read' : 'Mark unread'}</button><button type="button" className="text-button" disabled={busy} onClick={() => void messageAction(message, message.starred ? 'unstar' : 'star')}>{message.starred ? 'Unstar' : 'Star'}</button><button type="button" className="text-button" disabled={busy} onClick={() => void messageAction(message, 'archive')}>Archive</button><button type="button" className="text-button danger-text" disabled={busy} onClick={() => void messageAction(message, 'delete')}>Delete</button></div></div>}</article>; })}</div>}</section>{emailUi.composeOpen && <section className="attention-card settings-card email-compose-panel" aria-label="Compose message"><div className="section-title"><div><p className="eyebrow">SEND</p><h2>Compose message</h2></div><button className="email-compose-close" aria-label="Close compose" onClick={cancelCompose}>×</button></div><label>To<input type="email" value={compose.to} onChange={(event) => setCompose({...compose,to:event.target.value})} /></label><label>Subject<input value={compose.subject} onChange={(event) => setCompose({...compose,subject:event.target.value})} /></label><label>Message<textarea rows={7} value={compose.text} onChange={(event) => setCompose({...compose,text:event.target.value})} /></label><div className="email-compose-actions"><button className="primary" disabled={busy || !compose.to} onClick={() => void send()}>Send message</button><button className="secondary" disabled={busy} onClick={cancelCompose}>Cancel</button></div></section>}</>}
  </div>;
}
