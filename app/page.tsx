...
function EmailView({ state }: { state: OperationsState }) {
  ...
  const [intakeReview, setIntakeReview] = useState<{ proposal: EmailIntakeReviewProposal; mode: EmailIntakeReviewMode }>();
  ...
  function reviewIntake(message: EmailMessage) { if (!activeAccountId) return; setIntakeReview({ proposal: proposeEmailIntakeReview({ messageId:message.id, accountId:activeAccountId, from:{ name:message.fromName, address:message.from }, subject:message.subject, text:message.text, receivedAt:message.receivedAt }), mode:'review' }); }
